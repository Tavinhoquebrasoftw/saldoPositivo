...
import Db from '../../../../BackIn/GestaoB/DbCard';
...
const AnliseDeMercadoTraderEm = () => {
	...
	const handleAnswer = () => {
		...
		if (Db[currentQuestion].correctAnswer === selectedOption) {
			setScore(score + 1);
			setAnimation('bounce');
		} else {
			setFeedbackMessage(Db[currentQuestion].feedback || 'Resposta incorreta.');
	...
	const handleNextQuestion = () => {
		if (currentQuestion < Db.length - 1) {
	...
	useFocusEffect(
		useCallback (()=> {
			if(desafioComplete && !hasUpdatedScore){
				setHasUpdatedScore(true);
				handleFinishQuiz();
			}
	}, [desafioComplete, hasUpdatedScore])
);
	...
	return (
		<ImageBackground source={BackgroundImage} style={styles.container}>
			...
			<Animatable.View animation={animation} duration={500} style={styles.card}>
				...
				<Text style={styles.question}>{Db[currentQuestion].question}</Text>
				<ScrollView>
					{Db[currentQuestion].options.map((option: string, index: number) => (
						<TouchableOpacity
							key={index}
