...
import DbF from '../../../../BackIn/InvestimentosB/DbF';
...
const AnliseDeMercadoTraderEm = () => {
	...
	const handleAnswer = () => {
		...
		if (DbF[currentQuestion].correctAnswer === selectedOption) {
			setScore(score + 1);
			// playSound('correct'); 
			setAnimation('bounce');
		} else {
			setFeedbackMessage(DbF[currentQuestion].feedback || 'Resposta incorreta.');
	...
	const handleNextQuestion = () => {
		if (currentQuestion < DbF.length - 1) {
	...
	return (
		<ImageBackground source={BackgroundImage} style={styles.container}>
			...
			<Animatable.View animation={animation} duration={500} style={styles.card}>
				...
				<Text style={styles.question}>{DbF[currentQuestion].question}</Text>
				<ScrollView>
					{DbF[currentQuestion].options.map((option: string, index: number) => (
						<TouchableOpacity
							key={index}
