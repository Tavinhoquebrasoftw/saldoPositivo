...
import DbAuto from '../../../../BackIn/GestaoB/DbAuto'
...
const AnliseDeMercadoTraderEm = () => {
    ...
    const handleAnswer = () => {
        ...
        if (DbAuto[currentQuestion].correctAnswer === selectedOption) {
            setScore(score + 1);
            // playSound('correct'); 
            setAnimation('bounce');
        } else {
            setFeedbackMessage(DbAuto[currentQuestion].feedback || 'Resposta incorreta.');
    ...
    const handleNextQuestion = () => {
        if (currentQuestion < DbAuto.length - 1) {
    ...
    return (
        <ImageBackground source={BackgroundImage} style={styles.container}>
            ...
            <Animatable.View animation={animation} duration={500} style={styles.card}>
                ...
                <Text style={styles.question}>{DbAuto[currentQuestion].question}</Text>
                <ScrollView>
                    {DbAuto[currentQuestion].options.map((option: string, index: number) => (
                        <TouchableOpacity
                            key={index}
